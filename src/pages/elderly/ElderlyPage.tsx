import React from 'react';
import { Layout } from '../../components/Layout';
import { Card } from '../../components/Card';
import { Button } from '../../components/Button';
import { Phone, Users, ShoppingBag, Calendar, AlertCircle, MessageSquare, ArrowRight, Heart, Activity, Pill } from 'lucide-react';
import { motion } from 'motion/react';
import { useNavigate } from 'react-router-dom';
import { cn } from '../../utils/cn';
import { HealthLogger } from '../../components/HealthLogger';
import { MedicationTracker } from '../../components/MedicationTracker';
import { CareCircleMembers } from '../../components/CareCircleMembers';
import { auth } from '../../firebase';

export function ElderlyHome({ user }: { user: any }) {
  const navigate = useNavigate();

  return (
    <Layout userRole={user?.role}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="space-y-16"
      >
        {/* Welcome Section */}
        <section className="max-w-2xl">
          <span className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-sage-accent mb-4 block">
            Daily Overview
          </span>
          <h2 className="text-3xl lg:text-4xl font-serif font-bold text-slate-primary leading-[1.1] mb-6">
            Good morning, {user?.name?.split(' ')[0] || 'Friend'}.
          </h2>
          <p className="text-lg text-stone-500 font-sans leading-relaxed">
            Your schedule is clear for the next few hours. Here are your recommended actions for a balanced day.
          </p>
        </section>

        {/* Primary Action: Assistance */}
        <section>
          <div className="bg-white border border-stone-200 rounded-xl p-6 flex flex-col lg:flex-row items-center justify-between gap-6 shadow-sm">
            <div className="flex-1 space-y-1">
              <h3 className="text-xl font-serif font-bold text-slate-primary">Immediate Assistance</h3>
              <p className="text-base text-stone-500 max-w-lg">
                If you are feeling unwell or need urgent help, our care team is available 24/7.
              </p>
            </div>
            <Button 
              variant="danger" 
              size="md" 
              className="w-full lg:w-auto px-8"
              onClick={() => navigate('/help')}
            >
              Request Help
            </Button>
          </div>
        </section>

        {/* Health & Care Ecosystem */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-12">
            <section>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-2xl font-serif font-bold text-slate-primary">Health & Vitals</h3>
                  <p className="text-sm text-stone-500 mt-1">Track your daily wellness and share with your care team.</p>
                </div>
                <div className="hidden sm:flex items-center gap-2 text-sage-accent font-bold uppercase tracking-widest text-[9px] border border-sage-accent/20 px-2 py-0.5 rounded-full">
                  <Activity size={10} />
                  <span>Real-time Sync</span>
                </div>
              </div>
              <Card className="p-6">
                <HealthLogger elderlyId={auth.currentUser?.uid || ''} />
              </Card>
            </section>

            <section>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-2xl font-serif font-bold text-slate-primary">Medications</h3>
                  <p className="text-sm text-stone-500 mt-1">Your active prescriptions and reminders.</p>
                </div>
                <div className="hidden sm:flex items-center gap-2 text-terracotta font-bold uppercase tracking-widest text-[9px] border border-terracotta/20 px-2 py-0.5 rounded-full">
                  <Pill size={10} />
                  <span>Next dose in 2h</span>
                </div>
              </div>
              <MedicationTracker elderlyId={auth.currentUser?.uid || ''} />
            </section>
          </div>

          <div className="space-y-12">
            <section>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-serif font-bold text-slate-primary">Care Circle</h3>
                <div className="hidden sm:flex items-center gap-2 text-blue-600 font-bold uppercase tracking-widest text-[9px] border border-blue-200 px-2 py-0.5 rounded-full">
                  <Users size={10} />
                  <span>Active Team</span>
                </div>
              </div>
              <CareCircleMembers elderlyId={auth.currentUser?.uid || ''} />
            </section>

            <section className="space-y-4">
              <h3 className="text-2xl font-serif font-bold text-slate-primary">Quick Actions</h3>
              <div className="grid grid-cols-1 gap-3">
                <Card 
                  title="Community" 
                  description="Connect with local groups."
                  icon={<Users size={20} />}
                  className="p-5 hover:border-sage-accent cursor-pointer"
                  onClick={() => navigate('/social')}
                />
                <Card 
                  title="Services" 
                  description="Book medical transport."
                  icon={<ShoppingBag size={20} />}
                  className="p-5 hover:border-sage-accent cursor-pointer"
                  onClick={() => navigate('/services')}
                />
              </div>
            </section>
          </div>
        </div>
      </motion.div>
    </Layout>
  );
}
